...
export function createFlyers(scene, camera, opts = {}) {
	...
	const config = {
		...
		speed: clamp(Number(opts.speed ?? 1.2), 0.05, 6),
		targetSpeed: clamp(Number(opts.speed ?? 1.2), 0.05, 6),
	...
	function setSpeedTarget(nextSpeed, { rampSeconds = 0 } = {}) {
		config.targetSpeed = clamp(Number(nextSpeed ?? config.targetSpeed), 0.05, 6);
